...
function displayHistory(history) {
    ...
    if (history.length === 0) {
        historyList.innerHTML = '<p style="text-align: center; color: #7f8c8d;">Chưa có lịch sử</p>';
        return;
    }
    ...
    history.forEach(entry => {
        ...
        time.textContent = `${entry.time_elapsed.toFixed(3)}s - ${new Date(entry.timestamp).toLocaleString('vi-VN')}`;
